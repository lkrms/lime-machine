...
function do_mysql {
	...
	SOURCE_DB_LIST="$(mysql --host="$MYSQL_HOST" --port=$MYSQL_PORT --user="$SOURCE_USER" --password="$SOURCE_PASSWORD" --batch --skip-column-names --execute="show databases" 2>"$TEMP_FILE" | grep -Ev '^(information_schema|performance_schema|test|sys)$')"
