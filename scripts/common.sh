...
mkdir -p "$(dirname "$LOG_FILE")" || {
    echo "Error: $(dirname "$LOG_FILE") doesn't exist."
    exit 1
}
touch "$LOG_FILE" || {
    echo "Error: unable to open $LOG_FILE for writing."
    exit 1
}

mkdir -p "$RUN_DIR" || {
    echo "Error: $RUN_DIR doesn't exist."
    exit 1
}
[ -w "$RUN_DIR" ] || {
    echo "Error: unable to write to $RUN_DIR."
    exit 1
}
...
function close_targets() {
    ...
    while read -d $'\0' TARGET_FILE; do
        ...
        find "$TARGET_MOUNT_POINT/logs" -type f \( -name '*.log' -o -name '*.log.gz' \) -mtime +90 -delete
...
function get_used_space() {
    ...
    df --sync --output=pcent "$TARGET_MOUNT_POINT" | grep -Po '[[:space:]]*\K[0-9]+(?=%)'

}

function get_used_gb() {

    df --sync --output=used -BG "$TARGET_MOUNT_POINT" | grep -Po '[[:space:]]*\K[0-9]+(?=G)'
